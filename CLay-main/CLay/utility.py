...
def initStaticRandom():
    try:
        ...
        logging.error('Error: initStaticRandom %s', e)
...
def initRandom():
    try:
        ...
        logging.error('Error: initRandom %s', e)
        ...
        for target_path in url_target_paths:
            ...
            if match:
                try:
                    ...
                    logging.error('Error: addComment %s', e)
    except regex._regex_core.error as r:
        logging.error('Error: Regex path error %s', r)
...
def createCookieString(key, value=None, expire=None, others=None):
    try:
        ...
        logging.error('Error: createCookieString %s', e)
...
def statusCodeTampering(flow, response_code=None):
    try:
        ...
        logging.error('Error: statusCodeTampering %s', e)
...
def responseTemplating(status):
    try:
        ...
        logging.error('Error: responseTemplating %s', e)
